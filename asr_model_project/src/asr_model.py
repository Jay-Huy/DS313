...
class ASRModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, audio_features):
        """
        Args:
            acoustic_input: Input tensor for the acoustic encoder (seq_len, batch_size, acoustic_input_dim)
        ...
        """
        # Acoustic Encoder
        ae_outputs = self.acoustic_encoder(audio_features)
        
        # Contextual Encoder
        if self.contextual_encoder is not None: # If the model uses Structure A or B
            ce_outputs = self.contextual_encoder(ae_outputs)
        else:
